...
#define FILTERED_CLUSTER 100

CollapsedCellOptimizer::CollapsedCellOptimizer() {}

inline uint32_t c_get2 (uint32_t num) {
	if (num < 2) return 0;
	else if (num == 2) return 1;
	else return num * (num - 1) / 2;
}

inline bool check_script_exist(const std::string& name) {
    std::ifstream f(name.c_str());
    return f.good();
}

std::vector<Cell_cluster> read_cluster_file(std::string file_name, const std::vector<std::string>& trueBarcodes) {
	std::vector<Cell_cluster> vec;
  std::vector<std::string> vec_bc;
	std::ifstream cluster_ifs ( file_name , std::ifstream::in );
	std::string a_word, Cell_ID, cluster;
	cluster_ifs >> a_word;
	while (cluster_ifs >> a_word) {
		Cell_ID = a_word.substr(1, a_word.length() - 2);
    vec_bc.push_back(Cell_ID);
		cluster_ifs >> a_word;
		cluster = a_word.substr(1, a_word.length() - 2);
		uint32_t cluster_int = stoi(cluster);
		vec.push_back(Cell_cluster(cluster_int, Cell_ID));
	}
  for (size_t i = 0; i < trueBarcodes.size(); i++) {
    if (std::find(vec_bc.begin(), vec_bc.end(), trueBarcodes[i]) == vec_bc.end()) {
      Cell_ID = trueBarcodes[i];
      vec.push_back(Cell_cluster(FILTERED_CLUSTER, Cell_ID));
    }
  }
	std::sort(vec.begin(), vec.end(), std::greater<Cell_cluster>());
	return vec;	
}

double euclidDistance(const std::vector<double>& X, const std::vector<double>& Y, uint32_t n) {
	double dis = 0;
...
std::vector<double> DistanceToScore(const std::vector<double> & distance_vec) {
	std::vector<double> score_vec(distance_vec.size());
	double score_sum = 0;
	for (size_t i = 0; i < distance_vec.size(); i++) {
		// key function
		double score =  1 / (distance_vec[i] * distance_vec[i]);
		score_vec[i] = score;
		score_sum += score;
	}
	for (size_t i = 0; i < score_vec.size(); i++) {
		score_vec[i] /= score_sum;
	}
	return score_vec;
}

void read_pca_file(std::string filename, std::map<std::string, std::vector<double>>& PCA, CellTypePar& CTP) {
	std::ifstream ifs(filename);
	std::string bc;
	double pca_val;
	while (ifs >> bc) {
		std::vector<double> vec;
		for (size_t i = 0; i < CTP.pca_dim; i++) {
			ifs >> pca_val;
			vec.emplace_back(pca_val);
		}
		PCA[bc] = vec;
	}
}

void read_sig_pca(std::string sig_name, std::map<uint32_t, std::vector<double>> & sig_PCA, CellTypePar& CTP, uint32_t cluster_num) {
	std::ifstream ifs (sig_name);
	std::string unused;
	double exp;
...
void CellEMUpdate_(std::vector<SalmonEqClass>& eqVec,
  ...
  double gamma = CTP.gamma;
  if (CTP.doCelltypeQuan) {
    for (size_t i = 0; i < alphaOut.size(); i++) { 
      if (rare_gene_label[i] && !CTP.inGeneBlacklist[i]) {
        //alphaOut[i] = (1 - gamma * cur_cell.cluster_score) * alphaOut[i] \
        //+ gamma * cur_cell.cluster_score * Cell_type_profile[i];
				alphaOut[i] = (1 - gamma) * alphaOut[i] \
				+ gamma * Cell_type_profile[i];
      }
    }
  }
}
...
bool runPerCellEM(double& totalNumFrags, size_t numGenes,
                  ...
                  const std::vector<double>& Cell_type_profile,
                  const std::vector<bool>& rare_gene_label, CellTypePar& CTP,
                  const Cell_cluster& cur_cell){
  ...
  while (itNum < minIter or (itNum < maxIter and !converged)) {
    if (useVBEM) {
      ...
      CellEMUpdate_(salmonEqclasses, alphas, alphasPrime, Cell_type_profile, rare_gene_label, CTP, cur_cell);
    }
  ...
  if (CTP.write_iteration_count) {
    std::ofstream iter_file_out;
    iter_file_out.open(CTP.write_iteration_file_name, std::ios_base::app);
    iter_file_out << itNum << std::endl;
  }
  // Truncate tiny expression values
...
bool runBootstraps(size_t numGenes,
  ...
  while ( bsNum++ < numBootstraps) {
    ...
    // Just for padding
    std::vector<double> Cell_type_profile(numGenes, 0);
    std::vector<bool> rare_gene_label(numGenes, 0);
    CellTypePar CTP;
    CTP.doCelltypeQuan = false;
    CTP.gamma = 0.001;
    Cell_cluster cur_cell;
  ...
  while((trueBarcodeIdx = barcode++) < totalCells) {
    ...
    if ( !naiveEqclass ) {
      ...
      if ( !noEM ) {
        ...
        std::vector<double> Cell_profile(numGenes, 0);
        std::vector<double> Cell_type_profile(numGenes, 0);
        std::vector<bool> rare_gene_label(numGenes, 0);
        uint32_t cluster = 0;


        if ( GEP.count(trueBarcodeStr) == 0 && cur_iter != 1) {
          jointlog->warn("Can't found barcode {} in GEP \n", trueBarcodeStr);
          jointlog->flush();
        } 
        ...
        if (cur_iter != 1) {
          ...
          for (size_t i = 0; i < y_cells.size(); i++) {
            if (y_cells[i].name == trueBarcodeStr) {
              cur_cell = y_cells[i];
              break;
            } 
          }

          Cell_profile = GEP[trueBarcodeStr];
          if (cellIdxcluster.contains(trueBarcodeStr)) {
            cluster = cellIdxcluster[trueBarcodeStr];
            
            std::vector<double> tmp_Cell_profile(numGenes, 0);
            for (size_t i = 0; i < numGenes; i++) {
              tmp_Cell_profile[i] = Sig_mat[i][cluster];
            }
            std::sort(tmp_Cell_profile.begin(), tmp_Cell_profile.end());

            for (size_t i = 0; i < numGenes; i++) {
              Cell_type_profile[i] = Sig_mat[i][cluster];
              if (Sig_mat_proportion[i][cluster] > CTP.rare_gene_cut_off) {
                rare_gene_label[i] = true;
              }
            }
          } else {
            // jointlog->error("Can't find cluster of barcode {}\n", trueBarcodeStr);
            // jointlog->flush();
            // exit(1);
            if (CTP.writr_fileter_cells) {
              std::ofstream iter_file_out;
              iter_file_out.open(CTP.write_fileter_cells_file_name, std::ios_base::app);
              iter_file_out << trueBarcodeStr << std::endl;
            }
          }
        }
...
bool CollapsedCellOptimizer::optimize(EqMapT& fullEqMap,
  ...
  // may be called in writeMtx
  std::atomic<uint32_t> totalExpGeneCounts{0};
  std::map<std::string, std::vector<double>> GEP;
  spp::sparse_hash_map<std::string, uint32_t> cellIdxcluster;
  std::vector<std::vector<double>> Sig_mat;
  std::vector<std::vector<double>> Sig_mat_proportion;

  CellTypePar CTP;
  CTP.doCelltypeQuan = false;
  CTP.gamma = aopt.regularization_parameter;
  CTP.rare_gene_cut_off = aopt.rare_gene_cut_off;
	CTP.hasgeneBlacklist = boost::filesystem::exists(aopt.geneBlacklistFile);
	CTP.inGeneBlacklist.resize(numGenes, false);
	std::vector<Cell_cluster> y_cells;
  
  for (uint32_t cur_iter = 1; cur_iter <= max_iter_times; cur_iter++) {
    ...
    totalExpGeneCounts = 0;

    if (CTP.write_iteration_count) {
      CTP.write_iteration_file_name = aopt.outputDirectory.string() + "/iter_count.txt";
      std::ofstream iter_file_out;
      iter_file_out.open(CTP.write_iteration_file_name, std::ios_base::app);
      iter_file_out << "------------" << std::endl;
    }

    if (CTP.writr_fileter_cells) {
      CTP.write_fileter_cells_file_name = aopt.outputDirectory.string() + "/filtered_cells.txt";
      std::ofstream iter_file_out;
      iter_file_out.open(CTP.write_fileter_cells_file_name, std::ios_base::app);
      iter_file_out << "------------" << std::endl;
    }

    if (CTP.writr_cluster_score) {
      CTP.write_cluster_score_file_name = aopt.outputDirectory.string() + "/cluster_score.txt";
      std::ofstream iter_file_out;
      iter_file_out.open(CTP.write_cluster_score_file_name, std::ios_base::app);
      iter_file_out << "------------" << std::endl;
    }
    ...
    for (size_t tn = 0; tn < numWorkerThreads; ++tn) {
      workerThreads.emplace_back(optimizeCell,
                                ...
                                cur_iter, std::ref(GEP), 
                                std::ref(Sig_mat), 
                                std::ref(Sig_mat_proportion), 
                                std::ref(cellIdxcluster),
                                std::ref(CTP),
                                std::ref(y_cells));
    }
    ...
    if (cur_iter == 1) {
      ...
      gFile.close();
			...
			if (CTP.hasgeneBlacklist) {
				std::ifstream geneBlacklist_ifs(aopt.geneBlacklistFile.string(), std::ifstream::in);
				std::vector<std::string> geneBlacklist;
				std::string gene;
				while (geneBlacklist_ifs >> gene) {
					geneBlacklist.push_back(gene);	
				}
				geneBlacklist_ifs.close();
				uint32_t gene_in_Blacklist_length = 0;
				for (uint32_t i=0; i<numGenes; i++) {
					if (std::find(geneBlacklist.begin(), geneBlacklist.end(), geneNames[i]) != geneBlacklist.end()) {
						CTP.inGeneBlacklist[i] = true;
						gene_in_Blacklist_length += 1;
					}
				}
				std::cout << "Total blacklist gene counts: " << gene_in_Blacklist_length << '\n';
			}
    }
    ...
    if (script_fl) {
      ...
      if (script_output != 1) {
        aopt.jointLog->error("Seurat script return error.\n");
        aopt.jointLog->flush();       
        exit(1);
      }
    } else {
      aopt.jointLog->error("Seurat script path error.\n");
      aopt.jointLog->flush();
      exit(1);
      std::cout << "ERROR in Seurat" << std::endl;
    }
    
    // read cluster results
    std::string cluster_file = aopt.outputDirectory.string() + "/R_output/" + std::to_string(cur_iter) + "iters_cluster.txt";
    std::ifstream cluster_ifs ( cluster_file , std::ifstream::in );
    std::string a_word, Cell_ID, cluster;
    uint32_t cluster_int;
    ...
    while (cluster_ifs >> a_word) {
      ...
      cluster_ifs >> a_word;
      cluster = a_word.substr(1, a_word.length() - 2);
      cluster_int = stoi(cluster);
      cellIdxcluster[Cell_ID] = cluster_int;
      
      if (max_cluster <  cluster_int) {
        max_cluster = cluster_int;
      }
    } 
    max_cluster += 1;
    cluster_ifs.close();
    std::cout << "cluster results:" << std::endl;
    std::cout << "\tcluster numbers: " << max_cluster << std::endl;

    Sig_mat.resize(numGenes);
    Sig_mat_proportion.resize(numGenes);
    for (size_t i = 0; i < numGenes; ++i) {
      Sig_mat[i].resize(max_cluster, 0);
      Sig_mat_proportion[i].resize(max_cluster, 0);
    }
    cluster_count.resize(max_cluster);
    cluster_count.assign(max_cluster, 0);

    // Read Signature PCA and calculate Cell score
    y_cells = read_cluster_file(aopt.outputDirectory.string() + "/R_output/" + std::to_string(cur_iter) + "iters_cluster.txt", 
                                        trueBarcodes);
    std::map<std::string, std::vector<double>> PCA;
	  std::map<uint32_t, std::vector<double>> sig_PCA;
    std::string pca_filename(aopt.outputDirectory.string() + "/R_output/" + std::to_string(cur_iter) + "PCA_coordinate.txt");
    std::string sig_pca_filename(aopt.outputDirectory.string() + "/R_output/" + std::to_string(cur_iter) + "Sig_PCA.txt");
	  
    read_pca_file(pca_filename, PCA, CTP);
    read_sig_pca(sig_pca_filename, sig_PCA, CTP, max_cluster);

    for (size_t i = 0; i < y_cells.size(); i++) { 
      if (y_cells[i].cluster == FILTERED_CLUSTER) 
        continue;
		  for (size_t j = 0; j < max_cluster; j++) {
        double dis = euclidDistance(PCA[y_cells[i].name], sig_PCA[j], CTP.pca_dim);
        y_cells[i].pca_distance.emplace_back(dis);
		  }
		  y_cells[i].score = DistanceToScore(y_cells[i].pca_distance);
	  }

    uint32_t confidence_cell_count = 0;
	  uint32_t unconfidence_cell_count = 0;
    uint32_t filtered_cell_count = 0;
    ...
    for (size_t i = 0; i < y_cells.size(); i++) {
      ...
      uint32_t max_idx = 0;
      for (size_t j = 0; j < max_cluster; j++) {
        if (max_score < y_cells[i].score[j]) {
          max_score = y_cells[i].score[j];
          max_idx = j;
        }
      }
      if (max_idx == y_cells[i].cluster)
        confidence_cell_count += 1;
      else
        unconfidence_cell_count += 1;
    }

    std::cout << "\tConfidence cell counts: " << confidence_cell_count << std::endl;
    std::cout << "\tNot confidence cell counts: " << unconfidence_cell_count << std::endl;
    std::cout << "\tFiltered cell count: " << filtered_cell_count << std::endl;

    // calculate ARI score
    if (cur_iter != 1) {
      std::vector<Cell_cluster> x_cells = read_cluster_file(aopt.outputDirectory.string() + "/R_output/" + std::to_string(cur_iter - 1) + "iters_cluster.txt",
                                          trueBarcodes);
	    
      uint32_t x_group = max_cluster + 10; // suppose current cluster count + 10 > real previous cluster count
	    uint32_t y_group = max_cluster;
      std::vector<uint32_t> x;
      std::vector<uint32_t> y;
      assert(x_cells.size() == y_cells.size());
      for (size_t i = 0; i < x_cells.size(); i++) {
        x.push_back(x_cells[i].cluster);
        y.push_back(y_cells[i].cluster);
      }

      std::vector<std::vector<uint32_t>> conti_table;
      conti_table.resize(x_group, std::vector<uint32_t>(y_group));
      std::vector<uint32_t> x_sum(x_group, 0);
      std::vector<uint32_t> y_sum(y_group, 0);
      for (size_t i = 0; i < x_group; i++){
        for (size_t j = 0; j < y_group; j++){
          conti_table[i][j] = 0;
        }
      }
      for (size_t i = 0; i < x.size(); i++) {
        if (x[i] == FILTERED_CLUSTER || y[i] == FILTERED_CLUSTER ) continue;
        conti_table[x[i]][y[i]] += 1;
        x_sum[x[i]] += 1;
        y_sum[y[i]] += 1;
      }
      uint32_t n_2 = 0;
      ...
      for (size_t i = 0; i < x_group; i++) {
        for (size_t j = 0; j < y_group; j++) {
          n_2 += c_get2(conti_table[i][j]);
        }
      }
      double ARI_val = 0;
	    ARI_val = (n_2 - double((x_2 * y_2)) / double(c_get2(x.size()))) / (0.5 * (x_2 + y_2) - double((x_2 * y_2)) / double(c_get2(x.size())));
      std::cout << "\tARI: " << ARI_val << std::endl;
      CTP.ARI = ARI_val;
    }
    ...
    for (size_t i = 0; i < y_cells.size(); i++) {
      ...
      if (max_idx != y_cells[i].cluster) {
        y_cells[i].cluster_score = 0;
        y_cells[i].is_confidence = false;
        continue;
      } else {
        y_cells[i].cluster_score = max_score;
        y_cells[i].is_confidence = true;
        if (CTP.writr_fileter_cells) {
          std::ofstream iter_file_out;
          iter_file_out.open(CTP.write_cluster_score_file_name, std::ios_base::app);
          iter_file_out << max_score << std::endl;
        }
      }
      for (size_t geneIndex = 0; geneIndex < numGenes; geneIndex++) {
        Sig_mat[geneIndex][y_cells[i].cluster] += GEP[y_cells[i].name][geneIndex];
        if (GEP[y_cells[i].name][geneIndex] > 0) {
          Sig_mat_proportion[geneIndex][y_cells[i].cluster] += 1; 
        }
      }
      cluster_count[y_cells[i].cluster] += 1;
    }


    assert(Sig_mat.size() == numGenes);
    assert(Sig_mat[0].size() == max_cluster);
    assert(Sig_mat_proportion.size() == numGenes);
    assert(Sig_mat_proportion[0].size() == max_cluster);

    for (size_t tmpi = 0; tmpi < numGenes; tmpi ++) {
      for (size_t tmpj = 0; tmpj < max_cluster; tmpj++) {
        Sig_mat[tmpi][tmpj] /= cluster_count[tmpj];
        Sig_mat_proportion[tmpi][tmpj] /= cluster_count[tmpj];
      }
    }  

    //write signature file if need
    if (CTP.write_sig_file) {
      std::vector<std::string> geneNames(numGenes);
      for (auto geneIdx : geneIdxMap) {
        geneNames[geneIdx.second] = geneIdx.first;
      }
      std::ofstream sig_file;
      sig_file.open(aopt.outputDirectory.string() + "/" + std::to_string(cur_iter) + "sig_mat.txt");
      sig_file << "Gene" << ',';
      for (uint32_t tmpc = 0; tmpc < max_cluster - 1; tmpc ++) {
        sig_file << tmpc << ","; 
      }
      sig_file << max_cluster - 1 << std::endl; 

      for (size_t tmpi = 0; tmpi < numGenes; tmpi ++) {
        sig_file << geneNames[tmpi] << ',';
        for (size_t tmpj = 0; tmpj < max_cluster; tmpj++) {
          if (tmpj != max_cluster - 1)
            sig_file << Sig_mat[tmpi][tmpj] << ',';
          else
            sig_file << Sig_mat[tmpi][tmpj] << std::endl;
        }
      }
      sig_file.close();
    }
    ...
    // converge
    if (CTP.ARI > CTP.converge_ARI) {
      std::cout << "Total runnung iteration: " << cur_iter << std::endl;
      break;
    }
  }
