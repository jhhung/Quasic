...
void GZipWriter::flush_all() {
  if (countMatrixStream_) {
    countMatrixStream_->reset();
    countMatrixStream_ = nullptr;
  }
  if (tierMatrixStream_) {
    tierMatrixStream_->reset();
    tierMatrixStream_ = nullptr;
  }
  if (arboMatrixStream_) {
    arboMatrixStream_->reset();
    arboMatrixStream_ = nullptr;
  }
  if (bcFeaturesStream_) {
    bcFeaturesStream_.reset();
    bcFeaturesStream_ = nullptr;
  }
  if (bcNameStream_) {
    bcNameStream_.reset();
    bcNameStream_ = nullptr;
  }
}
...
bool GZipWriter::writeSparseAbundances(std::string& bcName,
                                       ...
                                       bool dumpUmiGraph, uint32_t cur_iter){
  ...
  if (!countMatrixStream_) {
    ...
    auto tierMatFilename = path_ / directory_name / "quants_tier_mat.gz";
    ...
    if (dumpArborescences) {
      ...
      auto arboMatFilename = path_ / directory_name / "arborescence_dump.txt.gz";
  ...
  if (!bcNameStream_) {
    auto bcNameFilename = path_ / directory_name / "quants_mat_rows.txt";
    bcNameStream_.reset(new std::ofstream);
    bcNameStream_->open(bcNameFilename.string());

    auto bcFeaturesFilename = path_ / directory_name / "featureDump.txt";
